...
function analyse(current, previous) {
  ...
  const prices=[]; groups(current.records.filter(r=>r.item&&r.qty&&r.rate),r=>clean(r.item)).forEach(rows=>{const avg=rows.reduce((s,r)=>s+r.total,0)/rows.reduce((s,r)=>s+r.qty,0);rows.forEach(r=>{if(r.rate>avg*1.2)prices.push({...r,avg,pct:(r.rate-avg)/avg})})});
  return {duplicates,vendors:compare("vendor",.2),items:compare("item",.25),prices:prices.sort((a,b)=>b.pct-a.pct)};
}
...
export default function Home(){
  ...
  const result=useMemo(()=>current&&previous?analyse(current,previous):null,[current,previous]);
  ...
  <><section className="run"><div><strong>Analysis ready</strong><small>{current.name} compared with {previous.name}</small></div><button onClick={()=>{setCurrent(null);setPrevious(null);setTab("Overview")}}>New review</button></section><nav>{["Overview","Duplicate bills","Vendor variation","Purchase variation","Price exceptions"].map(x=><button className={tab===x?"active":""} onClick={()=>setTab(x)} key={x}>{x}</button>)}</nav>
  ...
  {tab==="Vendor variation"&&<Changes title="Vendor variation" rows={result.vendors} field="Vendor"/>}{tab==="Purchase variation"&&<Changes title="Purchase variation" rows={result.items} field="Item"/>}{tab==="Price exceptions"&&<section className="panel"><Panel title="Items purchased above weighted average" badge="20%+ above average"/><Table head={["Item","Vendor","Rate paid","Weighted average","Variance"]}>{result.prices.length?result.prices.map((x,i)=><tr key={i}><td>{x.item}</td><td>{x.vendor}</td><td>{show(x.rate)}</td><td>{show(x.avg)}</td><td className="bad">+{(x.pct*100).toFixed(0)}%</td></tr>):<tr><td colSpan="5"><Empty>No price exceptions found.</Empty></td></tr>}</Table></section>}</>}</main>;
}
...
function Changes({title,rows,field}) { return <section className="panel"><Panel title={title} badge="20%+ change or new entry"/><Table head={[field,"Current month","Previous month","Change","Status"]}>{rows.length?rows.map((x,i)=><tr key={i}><td>{x.label}</td><td>{show(x.total)}</td><td>{show(x.old)}</td><td className={x.diff>=0?"bad":"good"}>{x.diff>=0?"+":""}{show(x.diff)}</td><td><b className="pill">{x.status}</b></td></tr>):<tr><td colSpan="5"><Empty>No material movements found.</Empty></td></tr>}</Table></section> }
